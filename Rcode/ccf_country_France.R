...
#3. Country filter
mycovid<-ts(poscovid$France, frequency = 7)
mygdelt<-ts(posgdelt$France, frequency = 7)
mytweet<-ts(postweet$France, frequency = 7)
...
xcroscovid_gdelt<-ccf(xresdiffcovid,yresdiffgdelt,na.action=na.omit, 
                      xlim=c(-3,3), ylim=c(-0.3,0.3), main="CCF Plot Covid vs GDELT-France", ylab="Correlation",
                      xlab="Time(weeks)")
...
xcroscovid_tweet<-ccf(xresdiffcovid,yresdifftweet,na.action=na.omit,
                      xlim=c(-3,3), ylim=c(-0.3,0.3), main="CCF Plot Covid vs Twitter-France", 
                      ylab="Correlation",
                      xlab="Time(weeks)")
...
tiff("France_coded.tiff", units="in", width=9.7, height=4.0, res=300)
...
text="(c) CCF Plot COVID-19 vs GDELT-France"
...
title (xlab="Time(weeks)", cex.lab=1.2, line=2.0)
title(ylab="Correlation", cex.lab=1.2, line=2.0)
text="(d) CCF Plot COVID-19 vs Twitter-France"
mtext(text, side = 3, line = 0, outer = FALSE, at =NA, adj =0, padj = 0, cex = 1.2, col = NA, font = 1)
