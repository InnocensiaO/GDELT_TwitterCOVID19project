...
mycovid<-ts(poscovid$Philippines, frequency = 7)
mygdelt<-ts(posgdelt$Philippines, frequency = 7)
mytweet<-ts(postweet$Philippines, frequency = 7)
...
Model1 <- VAR(v1, p = 2, type = "const", season = 7, exog = NULL) 
summary(Model1)
...
plot(xcroscovid_gdelt, xlim=c(0,20), ylim=c(-0.3,0.3),
     main="(a) CCF Plot Covid vs GDELT-Philippines", ylab="Correlation",
     xlab="Time(days)")
...
#Model1 <- VAR(v1, p = 8, type = "const", season = NULL, exog = NULL) 
Model2 <- VAR(v2, p =4, type = "const", season = 7, exog = NULL) 
summary(Model2)
...
plot(xcroscovid_tweet, xlim=c(0,20), ylim=c(-0.3,0.3),
     main="(b) CCF Plot Covid vs Twitter-Philippines", ylab="Correlation",
     xlab="Time(days)")
#plot(Model2.fit)
...
tiff("Philippinesvar.tiff", units="in", width=9.7,compression="lzw", height=4.0, res=300)
...
text="(a) CCF Plot COVID-19 vs GDELT-Philippines"
...
title (xlab="Time(weeks)", cex.lab=1.2, line=2.0)
title(ylab="Correlation", cex.lab=1.2, line=2.0)
text="(b) CCF Plot COVID-19 vs Twitter-Philippines"
mtext(text, side = 3, line = 0, outer = FALSE, at =NA, adj =0, padj = 0, cex = 1.2, col = NA, font = 1)
