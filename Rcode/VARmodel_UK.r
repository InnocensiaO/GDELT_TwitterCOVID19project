...
mycovid<-ts(poscovid$`United Kingdom`, frequency = 7)
mygdelt<-ts(posgdelt$`United Kingdom`, frequency = 7)
mytweet<-ts(postweet$`United Kingdom`, frequency = 7)
...
lagselect <- VARselect(v1, lag.max = 7, type = "both")
lagselect$selection
lagselect$criteria
...
Model1 <- VAR(v1, p = 7, type = "const", season = 7, exog = NULL) 
summary(Model1)
...
plot(xcroscovid_gdelt)
plot(xcroscovid_gdelt, xlim=c(0,20), ylim=c(-0.3,0.3),
     main="(a) CCF Plot Covid vs GDELT-U.K", ylab="Correlation",
     xlab="Time(days)")
...
#tweet vs covid for u.k not normal
...
plot(xcroscovid_tweet, xlim=c(0,20), ylim=c(-0.3,0.3),
     main="(b) CCF Plot Covid vs Twitter-U.K", ylab="Correlation",
     xlab="Time(days)")
#plot(Model2.fit)
...
tiff("UKvar.tiff", units="in", width=9.7, height=4.0,compression="lzw", res=300)
...
text="(a) CCF Plot COVID-19 vs GDELT-UK"
...
title (xlab="Time(weeks)", cex.lab=1.2, line=2.0)
title(ylab="Correlation", cex.lab=1.2, line=2.0)
text="(b) CCF Plot COVID-19 vs Twitter-UK"
mtext(text, side = 3, line = 0, outer = FALSE, at =NA, adj =0, padj = 0, cex = 1.2, col = NA, font = 1)
