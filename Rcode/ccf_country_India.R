...
#3. Country filter
mycovid<-ts(poscovid$India, frequency = 7)
mygdelt<-ts(posgdelt$India, frequency = 7)
mytweet<-ts(postweet$India, frequency = 7)
...
xcroscovid_gdelt<-ccf(xresdiffcovid,yresdiffgdelt,na.action=na.omit, 
                      xlim=c(-3,3), ylim=c(-0.3,0.3), main="CCF Plot Covid vs GDELT-India", ylab="Correlation",
                      xlab="Time(weeks)")
#?ccf
...
xcroscovid_tweet<-ccf(xresdiffcovid,yresdifftweet,na.action=na.omit,
                      xlim=c(-3,3), ylim=c(-0.3,0.3), main="CCF Plot Covid vs Twitter-India", 
                      ylab="Correlation",
                      xlab="Time(weeks)")
...
#?par
tiff("India.tiff", units="in", width=9.7, height=4.0, res=300)
...
text="(a) CCF Plot COVID-19 vs GDELT-India"
...
title (xlab="Time(weeks)", cex.lab=1.2, line=2.0)
title(ylab="Correlation", cex.lab=1.2, line=2.0)
text="(b) CCF Plot COVID-19 vs Twitter-India"
mtext(text, side = 3, line = 0, outer = FALSE, at =NA, adj =0, padj = 0, cex = 1.2, col = NA, font = 1)
