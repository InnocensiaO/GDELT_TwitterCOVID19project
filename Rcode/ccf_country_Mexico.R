...
#3. Country filter
mycovid<-ts(poscovid$Mexico, frequency = 7)
mygdelt<-ts(posgdelt$Mexico, frequency = 7)
mytweet<-ts(postweet$Mexico, frequency = 7)
...
xcroscovid_gdelt<-ccf(xresdiffcovid,yresdiffgdelt,na.action=na.omit, 
                      xlim=c(-3,3), ylim=c(-0.3,0.3), main="CCF Plot Covid vs GDELT-Mexico", ylab="Correlation",
                      xlab="Time(weeks)")
#?ccf
...
xcroscovid_tweet<-ccf(xresdiffcovid,yresdifftweet,na.action=na.omit,
                      xlim=c(-3,3), ylim=c(-0.3,0.3), main="CCF Plot Covid vs Twitter-Mexico", 
                      ylab="Correlation",
                      xlab="Time(weeks)")
...
#?par
tiff("Mexico.tiff", units="in", width=9.7, height=4.0, res=300)
...
text="(a) CCF Plot COVID-19 vs GDELT-Mexico"
...
title (xlab="Time(weeks)", cex.lab=1.2, line=2.0)
title(ylab="Correlation", cex.lab=1.2, line=2.0)
text="(b) CCF Plot COVID-19 vs Twitter-Mexico"
mtext(text, side = 3, line = 0, outer = FALSE, at =NA, adj =0, padj = 0, cex = 1.2, col = NA, font = 1)
